(* let flip f a b = f b a
   ...
   let render (w, h) step life =
     I.tabulate w (h - 1) (fun x y ->
         let pt = (x, y) in
         if CSet.mem pt life then dot else wall)
     <-> I.(
   ...
   let rec loop term (e, t) ((dim, n, life) as st) =
     e <?> t >>= function
     | `End | `Key (`Escape, []) | `Key (`ASCII 'C', [ `Ctrl ]) -> Lwt.return_unit
     | `Timer ->
         Term.image term (render dim n life) >>= fun () ->
         loop term (e, timer ()) (dim, n + 1, step (moebius dim) life)
     | `Mouse ((`Press `Left | `Drag), (x, y), _) ->
         loop term (event term, t) (dim, n, CSet.add (moebius dim (x, y)) life)
     | `Resize dim ->
         let life = CSet.map (moebius dim) life in
         Term.image term (render dim n life) >>= fun () ->
