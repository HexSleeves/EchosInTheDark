...
let menu header options width =
  ...
  let header_height =
    if String.length header = 0 then 0 else max header_height 1
  in
...
let main_menu () =
  ...
  let rec loop () =
    ...
    | false ->
        ...
        (* Game Title *)
        draw_text "TOMBS OF THE ANCIENT KINGS"
          ((screen_width / 2 * tile_width)
          - (measure_text "TOMBS OF THE ANCIENT KINGS" tile_height / 2))
          (((screen_height / 2) - 4) * tile_height)
          tile_height Color.yellow;

        draw_text "By me!"
          ((screen_width / 2 * tile_width)
          - (measure_text "By me!" tile_height / 2))
