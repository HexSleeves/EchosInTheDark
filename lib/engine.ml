...
module Entity = struct
  ...
  let by_name name =
    ...
    | None ->
        ...
        next_id := !next_id + 1;
        Hashtbl.add entities name new_id;
        new_id
end
...
module System1R = struct
  let create on_process (iter_a : (key -> 't_a -> unit) -> unit) =
    let module Def = struct
      let process_elements () = iter_a (fun key v_a -> on_process key v_a)
    end in
    (module Def : System)
end
...
module Scripting = struct
  let on_process key (f : key -> unit) = f key
